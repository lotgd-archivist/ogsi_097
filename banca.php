...
page_header("La Filiale della Vecchia Banca");
$session['user']['locazione'] = 107;
output("`^`c`bLa Filiale della Vecchia Banca`b`c`6");
if ($_GET[op]==""){
  checkday();
  ...
  output("nel suo registro.  ");
    ...
    if ($session[user][goldinbank]>=0){
        ...
        output("<form action='banca.php?op=transfer2' method='POST'>Quanto vuoi <u>t</u>rasferire: <input name='amount' id='amount' accesskey='h' width='5'>`n",true);
    ...
    if (db_num_rows($result)==1){
        ...
        output("<form action='banca.php?op=transfer3' method='POST'>",true);
        ...
        addnav("","banca.php?op=transfer3");
    }elseif(db_num_rows($result)>100){
        output("Il banchiere ti guarda disgustato e ti suggerisce di restringere il campo di ricerca almeno un po!`n`n");
        output("<form action='banca.php?op=transfer2' method='POST'>Transfer <u>h</u>ow much: <input name='amount' id='amount' accesskey='h' width='5' value='$amt'>`n",true);
  ...
  output("<form action='banca.php?op=depositfinish' method='POST'>Hai ".($session[user][goldinbank]>=0?"un saldo di":"un debito di")." ".abs($session[user][goldinbank])." pezzi d'oro.`n",true);
  ...
  addnav("","banca.php?op=depositfinish");
  ...
  output("<form action='banca.php?op=withdrawfinish' method='POST'>Hai ".($session[user][goldinbank]>=0?"un saldo di":"un debito di")." ".abs($session[user][goldinbank])." pezzi d'oro.`n",true);
  output("`^<u>Q</u>uanto vuoi chiedere in prestito (massimo $maxborrow  pezzi d'oro in tutto al tuo livello)? <input id='input' name='amount' width=5 accesskey='h'> <input type='hidden' name='borrow' value='x'><input type='submit' class='button' value='Chiedi Prestito'>`n(I soldi verranno prelevati dal tuo conto finché ne hai, il resto verrà preso in prestito)</form>",true);
    output("<script language='javascript'>document.getElementById('input').focus();</script>",true);
  addnav("","banca.php?op=withdrawfinish");
}else if($_GET[op]=="withdraw"){
  output("<form action='banca.php?op=withdrawfinish' method='POST'>Hai ".$session[user][goldinbank]." pezzi d'oro in banca.`n",true);
...
$rob = e_rand(1,10);
if ($rob > 7 and $session[user][robbank] <1)
      addnav("B?`4Deruba la Banca","robbank.php");
if ($session[user][goldinbank]>=0){
    addnav("Preleva","banca.php?op=withdraw");
    addnav("Deposita","banca.php?op=deposit");
    if (getsetting("borrowperlevel",20)) addnav("Prendi un Prestito","banca.php?op=borrow");
}else{
    addnav("Paga il Debito","banca.php?op=deposit");
    if (getsetting("borrowperlevel",20)) addnav("Chiedi un altro Prestito","banca.php?op=borrow");
}
if (getsetting("allowgoldtransfer",1)){
    if ($session[user][level]>=getsetting("mintransferlev",3) || $session[user][dragonkills]>0){
        addnav("Traferisci Denaro","banca.php?op=transfer");
    }
}
