...
if(strtotime("now")>$check) {
    ...
    for ($i=0; $i<$countrow; $i++){
        ...
        //db_query($sql);
        $sql = "SELECT * FROM houses WHERE owner=$row[acctid] AND status=1";
        $result1 = db_query($sql) or die(db_error(LINK));
        $countrow1 = db_num_rows($result1);
        for ($i=0; $i<$countrow1; $i++){
            $casadel = db_fetch_assoc($result1);
            $sqlf = "SELECT owner,gems FROM items WHERE value1=".$casadel[houseid]." AND class='key' AND ((owner=$row[acctid] AND value2=10) OR (owner<>$row[acctid])) ORDER BY id ASC";
