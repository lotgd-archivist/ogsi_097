...
if ($_POST['name']!=""){
    if (getsetting("manutenzione", 3) == 3) {
        if ($session['loggedin']){
            ...
            if(0){
                ...
                $sql = "SELECT * FROM accounts WHERE login = '".$_POST['name']."' AND password='$pass' AND locked=0";
                $result = db_query($sql);
                if (db_num_rows($result)>0){
                    $session['user']=db_fetch_assoc($result);
                    $sessionbackup=$session['user'];
                    debuglog("`^Si connette a LoGD. IP = ".$REMOTE_ADDR." (IP precedente: ".$session['user']['lastip'].",`n
    ...
    /*
    if ( (strtotime(date("r"))-strtotime($session['user']['lastlogin'])) > getsetting("onlinetime",7200) ) {
       $sql = "INSERT INTO furbetti (type,acctid,logintime,logouttime)
               VALUES ('time','".$session['user']['acctid']."',
    ...
    */
    if ($session['user']['alive']){
        if ($session['user']['jail']) {
            debuglog("va a dormire in prigione");
        }else debuglog("va a dormire nei campi");
    }elseif ($session['user']['loggedin']){
        debuglog("va a dormire da Ramius");
    }
    if ($session['user']['loggedin']){
        $session['user']['sconnesso'] = 0;
        $sql = "UPDATE accounts SET loggedin=0, sconnesso=0, locazione=0 WHERE acctid = ".$session['user']['acctid'];
        db_query($sql) or die(sql_error($sql));
    }
    $session=array();
    redirect("index.php");
