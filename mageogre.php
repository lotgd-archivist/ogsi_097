...
page_header("Mago Ogre");
if ($_GET['op']==""){
  output("`\$Devi combattere!");
        $badguy = array(
                      "creaturename"=>"`@Mago Ogre`0",
                      "creaturelevel"=>5,
                      "creatureweapon"=>"Magie Pericolose",
                      "creatureattack"=>8,
                      "creaturedefense"=>5,
                      "creaturehealth"=>100,
                      "diddamage"=>0
        ...
        if ($flawless) {
                ...
                debuglog("trova 5 gemme per combattimento perfetto con il Mago Ogre");
                $session['user']['gems']+= 5 ;
        }

        addnav("Continua","iltest.php?op=mageogrewin");
        }

if ($_GET['op']=="run"){
  output("Il Mago Ogre ti blocca la fuga!");
        $_GET['op']="fight";
}
...
if ($battle){
  include("battle.php");
        if ($victory){
                ...
                output("`2Hai vinto !! È stata dura ma sei riuscito ad ucciderlo. ");
                output("`n`n`2Hai guadagnato $exp punti di esperienza.`n  ");
                debuglog("guadagna $exp esperienza con il Mago Ogre");
                ...
                if($defeat){
                        ...
                        addnews("`%".$session['user']['name']."`5 è stato ucciso nel bosco a sud dal ".$badguy['creaturename']."`n $taunt");
                        $session['user']['alive']=false;
                        debuglog("perde ".$session['user']['gold']." oro ucciso dal Mago Ogre");
                        $session['user']['gold']=0;
