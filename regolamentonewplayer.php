...
/* Sook, vecchio regolamento - mantenuto per la modalità con cui distingueva i 2 server

    output("<big>`n`b`c`&REGOLAMENTO DI LEGEND OF THE GREEN DRAGON`0`c`b</big>`n`n`n",true);
if($nomedb=="logd2"){
    output("`#SU QUESTO SERVER È PERMESSO GESTIRE UN SOLO PERSONAGGIO PER GIOCATORE. PER INFORMAZIONI PIÙ
...
if($nomedb!="logd2"){
    ...
    personaggi. Non fa alcuna differenza se il passaggio favorisce il personaggio più sviluppato o
    ...
    output("`#NON è permesso creare più personaggi.`n`n`n`0");
}
...
if($nomedb!="logd2"){
    ...
    output("`#NON è permesso prestare i propri personaggi a terzi, sia per non perdere sessioni
...
Da qui nuovo regolamento*/
    ...
    output("`%`b5`b) `#Nel momento in cui due o più giocatori si trovino a giocare dallo stesso luogo (ad esempio 
            perché familiari), devono richiedere allo staff l'`b`@autorizzazione`b`# alla creazione dei loro personaggi, che 
    ...
    output("`%`b6`b) `#Allorché due o più giocatori `b`@normalmente estranei`b`# tra loro si trovino obbligati a giocare 
