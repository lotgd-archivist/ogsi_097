...
if ($battle){
  include("battle.php");
    if ($victory){
        ...
        output("`2Hai vinto è stata dura ma ci sei riuscito a ucciderli.`n");
        ...
        if($defeat){
            ...
            addnews("`%".$session['user']['name']."`5 è stato ucciso nella foresta a sud mentre combatteva con ".$badguy['creaturename']."`n$taunt");
            $session['user']['alive']=false;
            debuglog("ha perso ".$session['user']['gold']." oro quando è stato ucciso dai 4 Goblin");
            $session['user']['gold']=0;
