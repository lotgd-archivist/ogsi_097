...
page_header("Check Nuovo e_rand");
if (!is_array($result)) $result = array();
$result = unserialize($session['user']['specialmisc']);
switch($_GET['game']){
    case "":
    output("`2Quante iterazioni vuoi fare ?");
    output("<form action='testrand.php?game=numeri' method='POST'><input name='iter' id='iter'><input type='submit' class='button' value='Iterazioni'></form>",true);
    output("<script language='JavaScript'>document.getElementById('iter').focus();</script>",true);
    addnav("","testrand.php?game=numeri");
    break;
    case "numeri":
    $result['iter']= intval($_POST['iter']);
    output("`3Numero minimo del test - e_rand(`b`^X`b`3,x)`n");
    output("<form action='testrand.php?game=numeromax' method='POST'><input name='nummin' id='nummin'><input type='submit' class='button' value='Numero Minimo'></form>",true);
    output("<script language='JavaScript'>document.getElementById('nummin').focus();</script>",true);
    addnav("","testrand.php?game=numeromax");
    break;
    case "numeromax":
    $result['nummin']= intval($_POST['nummin']);
    output("`3Numero massimo del test - e_rand(x,`b`^X`b`3)`n");
    output("<form action='testrand.php?game=test' method='POST'><input name='nummax' id='nummax'><input type='submit' class='button' value='Numero Massimo'></form>",true);
    output("<script language='JavaScript'>document.getElementById('nummax').focus();</script>",true);
    addnav("","testrand.php?game=test");
    break;
    case "test";
    ...
    if ($result['iter'] > 1000000 OR $result['nummax'] > 50 OR $result['nummin'] < -50){
       ...
       $_GET['game'] = "";
       addnav("Riprova","testrand.php");
    }else{
        for ($x=$result['nummin'];$x<$result['iter'];$x++){
            $test = e_rand($result['nummin'],$result['nummax']);
            $result[$test]++;
        }
    ...
    addnav("Riprova","testrand.php");
    addnav("Passa al Vecchio","testrand1.php");
    break;
}
