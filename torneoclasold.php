...
addnav("G?`#Classifica Generale","torneoclasold.php?op=generale");
addnav("L?`%Classifica per Livello","torneoclasold.php?op=livello");
addnav("L?`^Livelli Completati","torneoclasold.php?op=fatti");
addnav("`@Classifiche Torneo Attuale","torneoclas.php");
...
if ($_GET[op]=="generale" OR $_GET['op']=="") {
    ...
    $sql = "SELECT b.name,a.torneo,a.torneopoints,b.superuser FROM torneopremi a, accounts b WHERE a.acctid=b.acctid AND a.torneo>0 AND b.superuser=0 ORDER BY a.torneo DESC";
...
if ($_GET[op]=="livello" && $_GET[op1]==""){
    ...
    for ($i=1; $i<16; $i++){
    addnav("`\$Livello $i","torneoclasold.php?op=livello&op1=".$i);
    }
}
if ($_GET[op]=="livello" && $_GET[op1]!=""){
    page_header("Classifica per Livello");
...
if ($_GET[op]=="fatti") {
    ...
    $sql = "SELECT torneopoints FROM torneopremi WHERE acctid = {$session['user']['acctid']}";
    $result = db_query($sql) or die(db_error(LINK));
    $row = db_fetch_assoc($result);
    $row[torneopoints]=unserialize($row[torneopoints]);
