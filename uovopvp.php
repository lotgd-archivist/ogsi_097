    ...
    if ($victory){
       ...
       if(db_num_rows($resultpun)>0) {
        ...
        if ($badguy['creaturegold']) {
            debuglog("`0guadagna ".$oroguadagna." oro per aver ucciso $killedin con PvP Uovo d'Oro",$badguy['acctid']);
            //$session['user']['gold']+=$badguy['creaturegold'];
            $session['user']['gold']+=$oroguadagna;
        }else {
            debuglog("`0uccide $killedin con PvP Uovo d'Oro",$badguy['acctid']);
        }
        // Add Bounty Gold - Darrell Morrone
        if ($badguy['creaturebounty']) {
            debuglog("`0guadagna ".$badguy['creaturebounty']." oro per di taglia per aver ucciso $killedin con PvP Uovo d'Oro",$badguy['acctid']);
...
// /\- Gunnar Kreitz
        ...
        if($defeat){
            ...
            if ($badguy['location']==1){
                ...
                $killedin="`6nella sua Casetta";
                $killedin1="`6nella tua Casetta";
            }elseif ($badguy['location']==3){
                $killedin="`6nel Dormitorio";
                $killedin1="`6nel Dormitorio";
            }elseif ($badguy['jail']==1){
            ...
            //debuglog("perso {$session['user']['gold']} oro quando è stato ucciso da ".$badguy['acctid']." PvP Uovo d'Oro");
            debuglog("`0ha attaccato in PvP Uovo d'Oro e ha perso ".$session['user']['gold']." oro, ucciso $killedin da", $badguy['acctid']);
            $session['user']['gold']=0;
