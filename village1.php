...
if (getsetting("nomedb","logd") == "logd2"){
    ...
    if ($session['user']['gdr'] == "si" AND $_GET['gdr'] == "") redirect("village.php?gdr=kkk");
    addnav("1?`%Piazza GDR","village.php?gdr=kkk");
}
addcommentary();
checkday();
...
if (getsetting("manutenzione",3) == 3 OR $session['user']['superuser'] > 2) {
  if($session['user']['prefs']['villaggio_nav']=="0") {
    addnav("F.A.Q.");
    addnav("H?`(Sezione HELP","help.php");
    addnav("La Foresta");
    ...
    if($_GET['sub']=="lame" OR !$_GET['sub'] OR $session['user']['prefs']['villaggio_nav']=="1") {
        ...
        addnav("L?`#Viale delle Lame`0","village1.php?sub=lame");
    }
    ...
    if($_GET['sub']=="mercato" OR $session['user']['prefs']['villaggio_nav']=="1") {
        ...
        addnav("M?Torre di Magia","mago.php");
        ...
        addnav("M?`#Piazza del Mercato`0","village1.php?sub=mercato");
    }
    ...
    if($_GET['sub']=="centro" OR $session['user']['prefs']['villaggio_nav']=="1") {
        ...
        addnav("C?`#Centro del paese`0","village1.php?sub=centro");
    }
    ...
    if($_GET['sub']=="taverna" OR $session['user']['prefs']['villaggio_nav']=="1") {
        ...
        addnav("T?`#Strada della Taverna`0","village1.php?sub=taverna");
    }
    ...
    if($_GET['sub']=="periferia" OR $session['user']['prefs']['villaggio_nav']=="1") {
        ...
        addnav("P?`#La Periferia`0","village1.php?sub=periferia");
    }
    ...
    if ($session['user']['dragonkills'] > 9 OR $session['user']['reincarna'] > 0) {
        if($_GET['sub']=="ippodromo" OR $session['user']['prefs']['villaggio_nav']=="1") {
            ...
            addnav("I?`#L'Ippodromo`0","village1.php?sub=ippodromo");
        }
    }
    ...
    if ($session['user']['superuser']>2) {
        ...
        addnav("£?Metti Manutenzione","village1.php?op=simanu");
        addnav("Togli Manutenzione","village1.php?op=nomanu");
    }
    ...
    addnav(".?Elenco Guerrieri","list.php");
    addnav("Età Player","eta.php");
    addnav("Q?`SW`bEsci`b`0 nei campi","login.php?op=logout",true);
    if ($session['user']['superuser']>=2){
        addnav("+?Trump Tower","trumptower.php");
...
viewcommentary("village1","Aggiungi",30,20);
